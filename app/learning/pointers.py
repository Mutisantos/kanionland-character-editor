...
# variables will share the same memory segment if they are copied. Modifying one will result in modifying their copies
print(f"{a_array}{id(a_array)} vs {b_array}{id(b_array)}")
c_array = a_array[:]
print(f"{a_array} vs {c_array}")
#In contrast, slicing decouples the copied reference in different memory segments
del a_array[a_array.index(3)]
...
#Iterator
#Iterators allow traversing through a collection without exposing the underlying structure
