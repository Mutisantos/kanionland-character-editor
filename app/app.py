...
# Function to create the SQLite database table (you can define your entities here)
def create_table():
...
def add_data():
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO example_entity (name, value) VALUES (?, ?)", (data['name'], data['value']))
        conn.commit()
